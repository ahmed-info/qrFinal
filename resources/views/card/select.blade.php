...
<html lang="en">
  <head>
    ...
    <script
      ...
      <script src="{{asset('qrcode.min.js')}}" type="text/javascript"></script>
    <!-- my custom css -->
  ...
  <body>
              ...
              <tr>
                ...
                {{-- <td>
                    ...
                    <img src="{{asset('qr_code/'.Str::slug($card->full_name).'.png')}}" alt="">
                                    {!! QrCode::errorCorrection('L')->size(150)->format('png')->encoding('UTF-8')->generate(
                ...
                <input type="hidden" id="qr-data-{{$card->id}}" value="{{$card->qr_code}}">

                <td id="qrcode-{{$card->id}}">
                </td>
    ...
    // table > rows[index=1] > childern[qrcode =4]
    
      for (var i=1; i < table.rows.length; i++) {
        //console.log(table.rows[i].children[4])
        var qrData = document.getElementById(`qr-data-${i}`);
        var qrcode = new QRCode(document.getElementById(`qrcode-${i}`), {
        width: 100,
        height: 100,
        correctLevel: QRCode.CorrectLevel.L,
      });
      generateQR(qrData.value) 
      }
